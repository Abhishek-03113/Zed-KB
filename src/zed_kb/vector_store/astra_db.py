...
class AstraDBStore(VectorStore):
    ...
    def similarity_search_with_security(
        ...
        **kwargs: Any,
        ...
        security_filter = self._create_security_filter(
            user_roles, user_clearance)
    ...
    def _create_security_filter(self, user_roles: List[str], user_clearance: str) -> Dict[str, Any]:
        ...
        security_levels = ["public", "internal",
                           "confidential", "restricted", "top_secret"]
        ...
        **kwargs: Any,
        ...
        try:
            ...
            try:
                ...
                # Default to balanced search
                hybrid_alpha = kwargs.get("hybrid_alpha", 0.5)
                if self.hybrid_search and query_text and 0 <= hybrid_alpha <= 1:
                    ...
                    logger.info(
                        f"Using hybrid search with alpha={hybrid_alpha}")

                # Perform the search
                results = list(self.collection.find(**find_params))
                logger.info(
                    f"Retrieved {len(results)} documents from AstraDB using vector search")

            except Exception as e:
                logger.warning(
                    f"Vector search failed, falling back to regular find: {e}")
                ...
                # Get more than we need to sort later
                query_params["options"] = {"limit": 100}

                matching_docs = list(self.collection.find(**query_params))

                logger.info(
                    f"Retrieved {len(matching_docs)} documents from AstraDB")

                # If we have no matches, return empty list
                if not matching_docs:
                    logger.warning(
                        "No matching documents found in AstraDB collection")
                    return []
                ...
                for doc in matching_docs:
                    ...
                    if isinstance(doc, str):
                        try:
                            ...
                            logger.warning(
                                f"Document couldn't be parsed as JSON: {doc[:100]}...")
                            continue
                ...
                results_with_scores.sort(key=lambda x: x.get(
                ...
                logger.info(
                    f"After processing, found {len(results)} valid results to return")
                ...
                if "user_roles" in kwargs:
                    ...
                    results = [
                        ...
                        if (
                            ...
                            # Empty role restrictions
                            not doc.get("allowed_roles") or
                            any(role in user_roles for role in doc.get(
                                "allowed_roles", []))  # Has matching role
                        )
                    ]
                    logger.info(
                        f"After role filtering, found {len(results)} results to return")
            ...
            try:
                ...
                if isinstance(item, str):
                    try:
                        ...
                        logger.warning(
                            f"Skipping result that couldn't be parsed: {item[:100]}...")
                        continue

                # Extract text content
                if "text" not in item:
                    logger.warning(
                        f"Skipping result without text field: {item.keys()}")
                    continue
                ...
                if "$similarity" in item:
                    ...
                    metadata["similarity"] = 1.0 - item["$vectorDistance"]
